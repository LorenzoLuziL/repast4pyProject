...
class Alpha(core.Agent):
    ...
    def step(self):
        ...
        if self.energy==0 :
            self.is_alive = False
        if model.runner.schedule.tick % 10:
            if self.energy > 0:
...
class Model:
    ...
    def __init__(self, comm: MPI.Intracomm, params):
        ...
        read_network(params['file_rete'], self.context, create_neuron_agent, restore_agent)
        ...
        for _ in range(10):
            ...
            alpha = Alpha(self.protein_counter, self.rank, pt, 5000, 10)
            alpha.in_CNS = False
        ...
        self.agent_cns_pos = logging.TabularLogger(comm, params['agent_pos_cns'], ['tick', 'agent_id', 'x', 'y','neuron_state'])
    ...
    def log(self):
        ...
        for agent in self.context.agents(Neuron.TYPE):
            self.agent_cns_pos.log_row(self.runner.schedule.tick,agent.TYPE,self.grid.get_location(agent).x,self.grid.get_location(agent).y,agent.state)
